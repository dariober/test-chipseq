...
rule all:
    input:
        expand('macs/{library_id}.narrowPeak', library_id=ss.get_chip_libraries()),
...
rule macs2:
    ...
    output:
        np='macs/{library_id}.narrowPeak'
    params:
        name=lambda wc, output: re.sub('\.narrowPeak$', '', output.np)
    shell:
        r"""
        macs2 callpeak -t {input.chip} -c {input.ctrl} -n {params.name}
        """
